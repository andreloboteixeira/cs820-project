...
In order to function, our algorithm needs to get some inputs from the graph generator which will guide it through making decisions. The algorithm treats each node as a joint node with some differences in two parameters, the first one represents the supply and the second represents the demand. For warehouse nodes, the demand is always set to zero; contrarily to stores, the supply is set to zero, and finally for joints, both supply and demand are set to zeros. 
\\ Another important factor for decision making is the cost of paths which consists of the summation of the distance and the time spent to go from node to node. 
For the reason of simplicity, the truck will initially start at node 0 which is the very first generated node and goes from there. The truck also has some configurations to be considered which are: the initial load of supply which we assumed to be zero at the beginning, as well as the maximum capacity of loaded goods. Moreover, we assigned a threshold value for the truck that serves as a cut off point to guide the truck either to go to a warehouse for loading or to go to a store for unloading goods.
...
\\The initial process of the algorithm will go as follow:
\\
\begin{itemize}
    \item{If the initial position of the truck is at a warehouse node with a \textless  \ capacity to the threshold : the truck will go to the nearest warehouse which is the current node and get loaded.}
    \item{If the initial position of the truck is at a joint node with a \textless \  capacity to the threshold: the truck will go to the nearest warehouse to get loaded with goods.}
    \item{If the initial position of the truck  is at a store node with a \textless  \ capacity to the threshold: the truck will go to the nearest warehouse to get loaded with goods.}
   \item{If the initial state of the truck is at a warehouse node with a \textgreater=  capacity to the threshold: the truck will go to the nearest store to unload goods.}
   \item{If the initial state of the truck is at a joint node with a \textgreater =  capacity to the threshold: the truck will go to the nearest store to unload goods.}
   \item{If the initial state of the truck is at a store node with a \textgreater =  capacity to the threshold: the truck will unload goods at the current node.}  
\end{itemize}
...
- helps in decision making of trucks capacity, by looking to the overal cost of the solution path given a truck capacity
- the algorithm give a solution which tries to fulfil the entire demand and minimize the overal path cost

Our algorithm does a good job in dealing with the goods transportation problem because it helps 
